...
class User:
    ...
    def verbosity(self, y: int) -> int:
        ...
        0
        """
        return sum(len(i.content) for i in self.tweets if i.created_at.year == y)
    ...
    def retweet(self, tweet: Tweet, date_tweeted: date = date.today()) -> Tweet:
        """Return a copy of the given tweet with the new user and date.
        Adds to user's tweet list
        ...
        'A'
        """
        new_tweet = Tweet(self.userid, date_tweeted, tweet.content)
        self.tweets.append(new_tweet)
        return new_tweet
