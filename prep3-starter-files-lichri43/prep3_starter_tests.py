...
def test_total_pay_fun() -> None:
    """
    Tests Employee.total_pay but... more fun ** :D **
    """

    emp = SalariedEmployee(24, 'Gilbert the cat', 1200.0)

    my_corp = Company([emp, HourlyEmployee(25, 'Chairman Meow', 500.25, 1.0)])
    my_corp.pay_all(date(2018, 6, 28))

    emp.pay(date(2018, 7, 28))

    assert emp.total_pay() == 200.0


def test_total_payroll_fun() -> None:
    """
    tests total payroll... but more fun
    """
    emp = SalariedEmployee(24, 'Gilbert the cat', 1200.0)
    ...
    assert my_corp.total_payroll() == 700.25
